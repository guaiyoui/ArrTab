...
# from tools.data_acquision.rel_graph import RelationGraph
...
def main(args):
    table2txt_dir = os.path.join("./tmp", args.dataset)
    out_dir = os.path.join(table2txt_dir, 'table2txt', args.experiment)
    ...
    if os.path.exists(out_passage_file):
        with open(out_passage_file, 'r', encoding='utf-8') as f:
    ...
    input_table_file = os.path.join(args.dataset_dir, args.dataset, 'datalake', table_file_name)
    ...
    for batch_table_lst in tqdm(read_tables(input_table_file, args.table_import_batch, args.table_chunk_size), total=num_batch):
        arg_info_lst = []
