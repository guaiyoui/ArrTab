...
def get_model(is_student, model_path, no_fp16, device):
    model = src.util.load_pretrained_retriever(is_student, model_path)
    model = model.to(device)
    model.eval()
    if not no_fp16 and device.type == "cuda":
        model = model.half()
    return model
...
def main(opt, index_obj=None):
    ...
    student_model = get_model(True, opt.student_model_path, opt.no_fp16, opt.device)
    teacher_model = get_model(False, opt.teacher_model_path, opt.no_fp16, opt.device)
