...
def log_metrics(epoc, metric_rec,
    ...
    for b_idx in range(batch_size):
        ...
        for idx, table in enumerate(sorted_tables):
            if table not in top_table_dict:
                top_table_dict[table] = sorted_ems[idx]
            if table not in selected_tables:
                selected_tables.append(table)

        # print(f"sorted_idxes: {sorted_idxes}")
        # print(f"sorted_ems: {sorted_ems}")
        # print(f"sorted_tables: {sorted_tables}")
        # print(f"top_metrics: {top_metrics}")
        print(f"selected_tables: {selected_tables[:10]}")
        selected_tables_batch.append(selected_tables[:10])
    ...
    # return selected_tables_batch
    return selected_tables[:10]
...
def evaluate(epoc, model, retr_model, dataset, dataloader, tokenizer, opt,
    ...
    # metric_rec = MetricRecorder([1, 5, 20, 50, 100, 200, 500, 1000, 2000, 5000])
    metric_rec = MetricRecorder([1, 5, 10])
    total_time = .0
    ...
    with torch.no_grad():
        ...
        for itr, fusion_batch in tqdm(enumerate(dataloader), total=num_batch, desc=bar_desc):
            ...
            # print(f"retr_scores: {retr_scores}")
            ...
            selected_tables = log_metrics(epoc, metric_rec, batch_data, retr_scores, batch_answers, time_span, total_time, (itr + 1), num_batch, loss=None, model_tag=model_tag, f_o_pred=f_o_pred)
            selected_tables_batch.append(selected_tables)
...
def main(opt, coreset_method=None):
    ...
    if opt.do_train:
        ...
        out_dir = os.path.join(opt.checkpoint_dir, opt.name)
        if not opt.multi_model_eval:
            selected_table = evaluate(0, model, retr_model,
                    eval_dataset, eval_dataloader,
