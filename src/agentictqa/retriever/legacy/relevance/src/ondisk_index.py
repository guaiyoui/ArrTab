...
class OndiskIndexer:
    ...
    def one_query_search(self, query, top_n=100, n_probe=128):
        ...
        # add this line since: in method 'IndexIVF_search', argument 3 of type 'float const *'
...
def index_data(index_file, data_file, index_out_dir, block_size=5000000):
    ...
    for emb_file in emb_file_lst:
        ...
        for idx in range(0, N, block_size):
            ...
            block_p_embs = np.asarray(block_p_embs, dtype=np.float32)
            block_p_ids = np.asarray(block_p_ids, dtype=np.int64)
...
def create_train(data_file, index_file):
    ...
    print(f"the number of files emb_file_lst: {len(emb_file_lst)}")
    ...
    t1 = time.time()
    train_all_embs = np.asarray(train_all_embs, dtype=np.float32)
...
def main(args):
    ...
    dataset_dir = os.path.join('./tmp', args.dataset, 'table2txt')
    exptr_dir = os.path.join(dataset_dir, args.experiment, 'emb')
    # data_file = os.path.join(exptr_dir, args.emb_file)
    data_file = args.emb_file
    trained_index_file = os.path.join(index_out_dir, 'trained.index')
    print(f"data_file: {data_file}")
    print(f"trained_index_file: {trained_index_file}")
