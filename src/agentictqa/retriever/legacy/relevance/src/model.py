...
class FiDT5(transformers.T5ForConditionalGeneration):
    ...
    def overwrite_forward_crossattention(self):
        ...
        """
        for layer_idx, mod in enumerate(self.decoder.block):
            attn = mod.layer[1].EncDecAttention
            attn.codex_layer_idx = layer_idx
...
def cross_attention_forward(
    ...
    **kwargs,
    ...
    def _cache_layer_idx():
        return getattr(self, "layer_idx", getattr(self, "codex_layer_idx", 0))

    def _unpack_cached_kv(cache_obj):
        if cache_obj is None:
            return None, None
        if isinstance(cache_obj, (tuple, list)) and len(cache_obj) >= 2:
            return cache_obj[0], cache_obj[1]
        if hasattr(cache_obj, "layers"):
            layers = getattr(cache_obj, "layers", None) or []
            layer_idx = _cache_layer_idx()
    ...
    def _update_cache(cache_obj, key_states, value_states):
        if cache_obj is None:
            return None
        layer_idx = _cache_layer_idx()
        if hasattr(cache_obj, "update"):
            try:
                cache_obj.update(key_states, value_states, layer_idx)
                return cache_obj
            except Exception:
                return None
        if hasattr(cache_obj, "cross_attention_cache"):
            try:
                cache_obj.cross_attention_cache = (key_states, value_states)
                return cache_obj
            except Exception:
                return None
        return None
    ...
    if past_key_value is not None:
        ...
        if hasattr(past_key_value, 'get_cross_attention_cache'):
            # 新的 EncoderDecoderCache API
            cached_kv = past_key_value.get_cross_attention_cache()
            key_states, value_states = _unpack_cached_kv(cached_kv)
        elif hasattr(past_key_value, 'cross_attention_cache'):
            # 检查 cross_attention_cache 属性
            key_states, value_states = _unpack_cached_kv(past_key_value.cross_attention_cache)
        ...
        elif EncoderDecoderCache and isinstance(past_key_value, EncoderDecoderCache):
            # 特殊处理 EncoderDecoderCache
            if hasattr(past_key_value, 'cross_attention_cache'):
                key_states, value_states = _unpack_cached_kv(past_key_value.cross_attention_cache)
    ...
    if use_cache:
        if past_key_value is not None:
            present_key_value = _update_cache(past_key_value, key_states, value_states)
            if present_key_value is None and hasattr(past_key_value, 'update_cross_attention_cache'):
                try:
                    past_key_value.update_cross_attention_cache(key_states, value_states)
            ...
            if EncoderDecoderCache:
                try:
                    new_cache = EncoderDecoderCache(DynamicCache(), DynamicCache())
                    present_key_value = _update_cache(new_cache, key_states, value_states)
                    if present_key_value is None:
                        present_key_value = (key_states, value_states)
                except Exception:
                    present_key_value = (key_states, value_states)
