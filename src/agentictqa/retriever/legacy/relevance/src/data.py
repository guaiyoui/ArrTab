...
def _batch_encode(tokenizer, texts, **kwargs):
...
def get_max_token_size(batch_text_passages, tokenizer, max_length):
    ...
    for k, text_passages in enumerate(batch_text_passages):
        try_encoded = _batch_encode(tokenizer, text_passages)['input_ids']
...
def encode_passages(batch_text_passages, tokenizer, max_length):
    flat_passages = [p for passages in batch_text_passages for p in passages]
    p = tokenizer(
        flat_passages,
    ...
    # 再 reshape 回原来的 batch 形状
    passage_ids = p['input_ids'].view(len(batch_text_passages), -1, max_length)
    passage_masks = p['attention_mask'].view(len(batch_text_passages), -1, max_length)
    ...
    #         padding=True,  # Changed from pad_to_max_length=True
    #         return_tensors='pt',
    #         truncation=True
    #     )
